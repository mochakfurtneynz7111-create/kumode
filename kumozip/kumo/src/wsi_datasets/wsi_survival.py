...
class WSIOmicsSurvivalDataset(WSISurvivalDataset):
    ...
    def _setup_pathways(self, type_of_path='combine', signature_path='./data_csvs/rna/metadata'):
        r"""
        ...
        self.signatures = pd.read_csv(os.path.join(signature_path, "combine_signatures.csv"))
        print(self.signatures.shape)
        print("self.signatures.shape")
        ...
        for col in self.signatures.columns:
            ...
            omic = sorted(_series_intersection(omic, self.omics_data.columns))
            # 🔧 只添加非空的 pathway
            if len(omic) > 0:  # 过滤掉空 pathway
                self.omic_names.append(omic)
            # self.omic_names.append(omic)
        self.omic_sizes = [len(omic) for omic in self.omic_names]
        print(f"Loaded {len(self.omic_sizes)} non-empty pathways from {len(self.signatures.columns)} total pathways")
